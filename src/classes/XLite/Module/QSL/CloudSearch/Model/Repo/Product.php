...
abstract class Product extends \XLite\Model\Repo\Product implements \XLite\Base\IDecorator
{
    ...
    {
        if ($value && \XLite\Module\QSL\CloudSearch\Main::doSearch() && !\XLite::isAdminZone()) {
            // We add the IDs additional condition
            $ids = \XLite\Module\QSL\CloudSearch\Core\ServiceApiClient::getInstance()->search($value);

            if (!empty($ids)) {
                $queryBuilder->andWhere('p.product_id IN (' . $queryBuilder->getInCondition($ids, 'arr') . ')');
            } else {
