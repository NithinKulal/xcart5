...
function func_amazon_pa_block_elm (elm_sel, block) {
  ...
  if (block) {
    var bl_div = jQuery('<div class="wait-block-overlay" id="'+bl_div_id+'"><div class="wait-block"><div></div></div></div>');
    var ofs = jQuery(elm_sel).offset();
    bl_div
      .css('width', jQuery(elm_sel).width())
      .css('height', jQuery(elm_sel).height())
      .css('left', ofs.left)
      .css('top', ofs.top)
      .css('z-index', '1010')
      .appendTo('body');
  } else {
    jQuery('#'+bl_div_id).remove();
  }
}
...
function func_amazon_pa_refresh_totals() {
    ...
    jQuery.get('cart.php?target=checkout&widget=\\XLite\\View\\Checkout\\CartItems&_='+Math.random(), function (data) {
      ...
      if (typeof DiscountCouponsView == 'function') {
        var view = new DiscountCouponsView(jQuery('.coupons'));
